...
SD_Error SD_Init(void)
{
  ...
  a=SD_GoIdleState();
  if(a) return a;/*There was an error, abort*/
  //Config the spi in fast mode - goidlestate leaves spi bus free
  uint16_t tmp;
  tmp = SD_SPI->CR1;
  /* Set fast clock (depends on the CSD) */
  tmp = ( tmp & ~SPI_BaudRatePrescaler_256 ) | SPI_BaudRatePrescaler_4;//18MHz
  SD_SPI->CR1 = tmp;
  return a;
}
